...
2. Have the agent inspect the schema first (`explain_schema`, plus `list_citation_schemas` / `get_citation_schema` when the question touches structured-extraction fields), then build and `validate_query` a structured query before running `query_analytics` for the aggregated metrics.
