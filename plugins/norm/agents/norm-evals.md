...
- Seeds: `GET /v1/evals/agent-templates` (built-ins), `GET|POST /v1/evals/user-templates` (org presets), and **`POST /v1/evals/agent-drafts` `{triage_issue_id, call_id?}`** — drafts a judge straight from a triage issue's failure mode + call evidence (beta).
