...
- **`dimensions`** (≤10): a bare column string; `{ col, label? }`; a time bucket `{ col: <timestamp col>, trunc: hour|day|week|month|year }` on `created_at/updated_at/started_at/transferred_at`; `{ col: <variable>, source: "citation" }`; or `{ col: <result key>, source: "disposition" }`. **Known gap (verified live): the `query_analytics` MCP tool currently DROPS `trunc` (returns raw per-row timestamps); the REST layer honors it. For time-bucketed results, bucket the returned rows yourself, or build the query as a dashboard panel (which preserves `trunc`).**
