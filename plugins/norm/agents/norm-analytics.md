...
- `explain_schema` — the queryable tables, their columns, types, and join requirements.
- `validate_query` — checks a structured query for shape/reference errors before you run it.
- `query_analytics` — runs the query: aggregated metrics with filters and group-by.
- `list_citation_schemas` / `get_citation_schema` — the structured-extraction fields available on calls (what was captured per call, so you can filter or group by extracted values).
...
2. Call `explain_schema` first. Read the tables, columns, types, and join requirements before composing anything — never query from assumption about what fields exist.
3. If the question touches structured-extraction fields (a value captured per call, not a built-in column), call `list_citation_schemas` and `get_citation_schema` to confirm the exact field names and types available.
4. Build a structured query — the right table(s), filters, group-by, and aggregation — and `validate_query` it. Fix any reference or shape errors and re-validate until clean.
5. Run `query_analytics` to get the aggregated metrics. If a result is empty or surprising, widen the range or relax a filter and re-run rather than reporting a guess.
...
Every tool in this domain is read-only — schema inspection, query validation, analytics queries, and report generation/editing never mutate production, place calls, send messages, or cost the user anything. So none of them require confirmation; run them freely.
