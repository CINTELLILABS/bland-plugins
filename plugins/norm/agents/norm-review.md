...
4. **Read in evidence order**: metadata first (`status`, `answered_by`, `call_ended_by`, `error_message`, duration — a 4-second `voicemail` call needs no transcript analysis — and its fix is almost always the `voicemail` settings on the call, not the pathway), then transcript for the human story, then pathway_logs — **markers first** (non-2xx webhooks, `ERROR`, failed custom code, looping ≥3).
