...
- **From runs to calls**: harvest placed-call ids from each execution's `result` and hand cohorts to `/norm:review` (call quality) or `/norm:analytics` (trends) — automation health and call health are different questions; answer both when the user asks "is it working".
