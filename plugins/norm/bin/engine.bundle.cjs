...
// apps/api/src/lib/blandcode/engine/generator.ts
...
function buildSlugMap(nodes) {
  ...
  for (const node of nodes) {
    if ("globalConfig" in node) continue;
    const data = node.data;
    if (!data) continue;
    const shortId = nodeShortId(node.id);
    let finalSlug = shortId;
    let counter = 1;
    while (usedSlugs.has(finalSlug)) {
      finalSlug = `${shortId}-${counter}`;
      counter++;
    }
    usedSlugs.add(finalSlug);
    idToSlug.set(node.id, finalSlug);
  }
  return idToSlug;
}
function toYaml(obj) {
  return (0, import_yaml.stringify)(obj, { lineWidth: 0 });
}
function isGlobalConfigNode(node) {
  return "globalConfig" in node;
}
function generateNodeMd(node) {
  ...
  addTypeSpecificFields(node, data, frontmatter);
  if (data.dialogueExamples && data.dialogueExamples.length > 0) {
    frontmatter.dialogueExamples = data.dialogueExamples;
  }
  const body = data.prompt ?? data.text ?? "";
  return `---
${toYaml(frontmatter)}---
...
function addTypeSpecificFields(node, data, frontmatter) {
  const nodeType = node.type;
  if (nodeType === "Webhook") {
    if (data.url) frontmatter.url = data.url;
    if (data.method) frontmatter.method = data.method;
    if (data.body !== void 0 && data.body !== null) {
      frontmatter.body = typeof data.body === "object" ? JSON.stringify(data.body) : data.body;
    }
    if (data.headers && Array.isArray(data.headers) && data.headers.length > 0) {
      frontmatter.headers = data.headers.map(
        (h) => ({ key: h[0], value: h[1] })
      );
    }
    if (data.auth) frontmatter.auth = data.auth;
    if (data.timeoutValue) frontmatter.timeoutValue = data.timeoutValue;
    if (data.max_retries) frontmatter.max_retries = data.max_retries;
    if (data.responseData && Array.isArray(data.responseData) && data.responseData.length > 0) {
      frontmatter.responseData = data.responseData;
    }
    if (data.responsePathways && Array.isArray(data.responsePathways) && data.responsePathways.length > 0) {
      frontmatter.responsePathways = data.responsePathways.map((rp) => ({
        variable: rp[0],
        condition: rp[1],
        value: rp[2],
        targetId: rp[3]?.id ?? "",
        targetName: rp[3]?.name ?? ""
      }));
    }
  }
  if (nodeType === "Knowledge Base" || nodeType === "Vector DB Knowledge Base") {
    if (data.kb) frontmatter.kb = data.kb;
  }
  if (nodeType === "Transfer Call") {
    if (data.transferNumber)
      frontmatter.transferNumber = data.transferNumber;
    if (data.warmTransferFields)
      frontmatter.warmTransferFields = data.warmTransferFields;
    if (data.twilioAppTransferFields)
      frontmatter.twilioAppTransferFields = data.twilioAppTransferFields;
  }
  if (nodeType === "SMS") {
    if (data.phoneNumber) frontmatter.phoneNumber = data.phoneNumber;
    if (data.smsBody) frontmatter.smsBody = data.smsBody;
  }
  if (nodeType === "Custom Tool") {
    if (data.tool) frontmatter.tool = data.tool;
    if (data.speech) frontmatter.speech = data.speech;
    if (data.responsePathways && Array.isArray(data.responsePathways) && data.responsePathways.length > 0) {
      frontmatter.responsePathways = data.responsePathways.map((rp) => ({
        variable: rp[0],
        condition: rp[1],
  ...
  if (nodeType === "Custom Code") {
    if (data.code) frontmatter.code = data.code;
    const snippetId = data.snippetId ?? data.snippet_id;
    const snippetVersion = data.snippetVersion ?? data.snippet_version;
    const snippetVariables = data.snippetVariables ?? data.snippet_variables;
    if (snippetId !== void 0 && snippetId !== null)
      frontmatter.snippetId = snippetId;
    if (snippetVersion !== void 0 && snippetVersion !== null)
      frontmatter.snippetVersion = snippetVersion;
    if (snippetVariables && typeof snippetVariables === "object" && Object.keys(snippetVariables).length > 0)
      frontmatter.snippetVariables = snippetVariables;
  }
  if (nodeType === "Transfer Pathway") {
    if (data.transferPathway) frontmatter.transferPathway = data.transferPathway;
    if (data.version) frontmatter.version = data.version;
    if (data.transferType) frontmatter.transferType = data.transferType;
    if (data.transferPathwayNode) frontmatter.transferPathwayNode = data.transferPathwayNode;
  }
  if (nodeType === "Press Button") {
    if (data.mode) frontmatter.mode = data.mode;
    if (data.digit !== void 0) frontmatter.digit = data.digit;
    if (data.sequence !== void 0) frontmatter.sequence = data.sequence;
    if (data.digitPrompt !== void 0) frontmatter.digitPrompt = data.digitPrompt;
  }
  if (nodeType === "Route") {
    if (data.routes && Array.isArray(data.routes) && data.routes.length > 0) {
      frontmatter.routes = data.routes;
    }
    if (data.fallbackNodeId !== void 0) {
      frontmatter.fallbackNodeId = data.fallbackNodeId;
    }
  }
  if (nodeType === "Schedule Meeting" || nodeType === "Scheduling") {
    if (data.calendarId) frontmatter.calendarId = data.calendarId;
  }
}
function generateConditionMd(condition, conditionExamples) {
  const frontmatter = {};
  if (conditionExamples && conditionExamples.length > 0) {
...
function generateVariablesYaml(extractVars) {
  const variables = extractVars.map((v) => {
    ...
    if (v.length > 3) {
      entry._extra = v.slice(3);
    }
    return entry;
  });
  return toYaml({ variables });
}
function generateModelYaml(modelOptions) {
  return toYaml(modelOptions);
}
function generateEdgeMd(edge, idToSlug, idToName) {
  ...
  const frontmatter = {
    id: edgeRecord.id ?? `${edge.source}-to-${edge.target}`,
    source: sourceSlug,
    target: targetSlug,
  ...
  if (edgeData?.isHighlighted !== void 0) {
    frontmatter.highlighted = edgeData.isHighlighted;
  }
...
function generateLayoutYaml(nodes, idToSlug) {
  const positions = {};
  for (const node of nodes) {
    if (isGlobalConfigNode(node)) continue;
    const rNode = node;
    if (!rNode.position) continue;
    const slug = idToSlug.get(rNode.id);
    if (slug) {
      positions[slug] = { x: rNode.position.x, y: rNode.position.y };
    }
  }
  return toYaml({ positions });
}
function generateFiles(nodes, edges) {
  ...
  for (const node of nodes) {
    if (isGlobalConfigNode(node)) continue;
    if (node.data?.name) idToName.set(node.id, node.data.name);
  }
  let globalPrompt = "";
  for (const node of nodes) {
    if (isGlobalConfigNode(node)) {
      const prompt = node.globalConfig.globalPrompt ?? "";
      if (prompt && !globalPrompt) {
        globalPrompt = prompt;
      }
    }
  }
  for (const node of nodes) {
    ...
    if (!slug) continue;
    files.push({
      path: `nodes/${slug}/node.md`,
      content: generateNodeMd(node)
    });
    if (data.condition) {
      files.push({
        path: `nodes/${slug}/condition.md`,
        content: generateConditionMd(
          data.condition,
          data.conditionExamples
        )
      });
    }
    if (data.extractVars && data.extractVars.length > 0) {
      files.push({
        path: `nodes/${slug}/variables.yaml`,
        content: generateVariablesYaml(
          data.extractVars
        )
      });
    }
    if (data.modelOptions && Object.keys(data.modelOptions).length > 0) {
      files.push({
        path: `nodes/${slug}/model.yaml`,
        content: generateModelYaml(
          data.modelOptions
        )
      });
    }
    const unitTests = data.unitTests;
    if (unitTests && Object.keys(unitTests).length > 0) {
      files.push({
        path: `nodes/${slug}/unit-tests.yaml`,
        content: toYaml(unitTests)
      });
    }
    const tag = data.tag;
    if (tag?.name) {
      files.push({
        path: `nodes/${slug}/tag.yaml`,
        content: toYaml(tag)
      });
    }
    ...
    if (tools && Array.isArray(tools) && tools.length > 0) {
      files.push({
        path: `nodes/${slug}/tools.yaml`,
        content: toYaml({ tools })
      });
    }
  }
  ...
  files.push({
    path: ".pathways/config.yaml",
    content: generateConfigYaml()
  });
  files.push({
    path: ".pathways/layout.yaml",
    content: generateLayoutYaml(nodes, idToSlug)
  });
  files.push({
...
function* FromIntersect3(schema, references, path, value) {
  ...
  for (const inner of schema.allOf) {
    for (const error of Visit4(inner, references, path, value)) {
      hasError = true;
      yield error;
    }
  }
...
function FromObject4(schema, references, path, value) {
  ...
  for (const key of knownKeys) {
    if (!HasPropertyKey(knownProperties, key))
      continue;
    if (IsUndefined(knownProperties[key]) && (!IsUndefined4(schema.properties[key]) || TypeSystemPolicy.IsExactOptionalProperty(knownProperties, key)))
      continue;
...
function Visit7(schema, references, path, value) {
  ...
  switch (schema[Kind]) {
    ...
    case "Ref":
      return FromRef3(schema_, references_, path, value);
    case "Symbol":
...
function FromArray8(schema, references) {
  return IsTransform2(schema) || Visit9(schema.items, references);
}
function FromAsyncIterator4(schema, references) {
  return IsTransform2(schema) || Visit9(schema.items, references);
}
function FromConstructor4(schema, references) {
  return IsTransform2(schema) || Visit9(schema.returns, references) || schema.parameters.some((schema2) => Visit9(schema2, references));
}
function FromFunction4(schema, references) {
  return IsTransform2(schema) || Visit9(schema.returns, references) || schema.parameters.some((schema2) => Visit9(schema2, references));
}
function FromIntersect8(schema, references) {
  return IsTransform2(schema) || IsTransform2(schema.unevaluatedProperties) || schema.allOf.some((schema2) => Visit9(schema2, references));
}
function FromIterator4(schema, references) {
  return IsTransform2(schema) || Visit9(schema.items, references);
}
function FromNot6(schema, references) {
  return IsTransform2(schema) || Visit9(schema.not, references);
}
function FromObject6(schema, references) {
  return IsTransform2(schema) || Object.values(schema.properties).some((schema2) => Visit9(schema2, references)) || IsSchema2(schema.additionalProperties) && Visit9(schema.additionalProperties, references);
}
function FromPromise4(schema, references) {
  return IsTransform2(schema) || Visit9(schema.item, references);
}
function FromRecord6(schema, references) {
  const pattern = Object.getOwnPropertyNames(schema.patternProperties)[0];
  const property = schema.patternProperties[pattern];
  return IsTransform2(schema) || Visit9(property, references) || IsSchema2(schema.additionalProperties) && IsTransform2(schema.additionalProperties);
}
...
function FromTuple8(schema, references) {
  return IsTransform2(schema) || !IsUndefined(schema.items) && schema.items.some((schema2) => Visit9(schema2, references));
}
function FromUnion10(schema, references) {
  return IsTransform2(schema) || schema.anyOf.some((schema2) => Visit9(schema2, references));
}
...
// apps/api/src/lib/blandcode/engine/parser.ts
var import_yaml2 = __toESM(require_dist());
function parseFile(content) {
  const match = content.match(/^---\n([\s\S]*?)\n?---\n?([\s\S]*)$/);
  if (!match) {
    throw new Error("Invalid file: missing frontmatter delimiters");
  }
  return {
...
// apps/api/src/lib/blandcode/engine/manifest.ts
function buildLiveManifest(fileMap) {
  const nodes = [];
  const edges = [];
  for (const [path, content] of fileMap) {
    const nodeMatch = path.match(/^nodes\/([^/]+)\/node\.md$/);
    if (nodeMatch) {
      try {
    ...
    if (path.startsWith("edges/") && path.endsWith(".md")) {
      try {
        const { frontmatter, body } = parseFile(content);
        edges.push({
          id: frontmatter.id,
          sourceSlug: frontmatter.source,
          targetSlug: frontmatter.target,
          label: body || ""
        });
      } catch {
      }
...
function buildIdSlugMap(manifest) {
...
function FromIntersect9(T) {
  return Intersect(FromRest4(T));
}
function FromUnion11(T) {
  return Union(FromRest4(T));
}
function FromPromise5(T) {
  return AwaitedResolve(T);
}
function AwaitedResolve(T) {
  return IsIntersect(T) ? FromIntersect9(T.allOf) : IsUnion(T) ? FromUnion11(T.anyOf) : IsPromise2(T) ? FromPromise5(T.item) : T;
}
...
function DerefResolve(schema, references) {
  return IsConstructor(schema) ? FromConstructor5(schema, references) : IsFunction3(schema) ? FromFunction5(schema, references) : IsIntersect(schema) ? FromIntersect10(schema, references) : IsUnion(schema) ? FromUnion12(schema, references) : IsTuple(schema) ? FromTuple9(schema, references) : IsArray3(schema) ? FromArray10(schema, references) : IsObject3(schema) ? FromObject7(schema, references) : IsPromise2(schema) ? FromPromise6(schema, references) : IsAsyncIterator3(schema) ? FromAsyncIterator5(schema, references) : IsIterator3(schema) ? FromIterator5(schema, references) : IsRef(schema) ? FromRef6(schema, references) : schema;
}
...
function Intrinsic(schema, mode, options = {}) {
  return (
    ...
    IsMappedKey(schema) ? IntrinsicFromMappedKey(schema, mode, options) : (
      // Standard-Inference
      IsTemplateLiteral(schema) ? FromTemplateLiteral5(schema, mode, options) : IsUnion(schema) ? Union(FromRest6(schema.anyOf, mode), options) : IsLiteral(schema) ? Literal(FromLiteralValue(schema.const, mode), options) : (
        // Default Type
...
function FromIntersect11(T, K) {
  return T.map((T2) => OmitResolve(T2, K));
}
function FromUnion13(T, K) {
  return T.map((T2) => OmitResolve(T2, K));
}
...
function FromObject8(T, K) {
  const options = Discard(T, [TransformKind, "$id", "required", "properties"]);
  const properties = FromProperties13(T["properties"], K);
  return Object2(properties, options);
}
function OmitResolve(T, K) {
  return IsIntersect(T) ? Intersect(FromIntersect11(T.allOf, K)) : IsUnion(T) ? Union(FromUnion13(T.anyOf, K)) : IsObject3(T) ? FromObject8(T, K) : Object2({});
}
...
function FromObject9(T) {
  const options = Discard(T, [TransformKind, "$id", "required", "properties"]);
  const properties = FromProperties14(T["properties"]);
  return Object2(properties, options);
}
function PartialResolve(T) {
  return IsIntersect(T) ? Intersect(FromRest7(T.allOf)) : IsUnion(T) ? Union(FromRest7(T.anyOf)) : IsObject3(T) ? FromObject9(T) : Object2({});
}
...
function FromObject10(T, K) {
  const options = Discard(T, [TransformKind, "$id", "required", "properties"]);
  const properties = FromProperties17(T["properties"], K);
  return Object2(properties, options);
}
function PickResolve(T, K) {
  return IsIntersect(T) ? Intersect(FromIntersect12(T.allOf, K)) : IsUnion(T) ? Union(FromUnion14(T.anyOf, K)) : IsObject3(T) ? FromObject10(T, K) : Object2({});
}
...
function FromObject11(T) {
  const options = Discard(T, [TransformKind, "$id", "required", "properties"]);
  const properties = FromProperties18(T["properties"]);
  return Object2(properties, options);
}
function RequiredResolve(T) {
  return IsIntersect(T) ? Intersect(FromRest8(T.allOf)) : IsUnion(T) ? Union(FromRest8(T.anyOf)) : IsObject3(T) ? FromObject11(T) : Object2({});
}
...
// norm-stub:stub:models
var NodeTypeSchema = Type.Any();

// norm-stub:stub:logger
function bclog() {
}

// apps/api/src/lib/blandcode/engine/exporter.ts
var typeValidator = TypeCompiler.Compile(NodeTypeSchema);
function exportToJSON(fileMap) {
  const manifest = buildLiveManifest(fileMap);
  const { slugToId } = buildIdSlugMap(manifest);
  const positions = parsePositions(fileMap);
  const globalConfig = parseGlobalConfig(fileMap);
  const nodes = [];
  const edges = [];
  for (const [path, content] of fileMap) {
    const nodeMatch = path.match(/^nodes\/([^/]+)\/node\.md$/);
    ...
    const node = parseNodeToJSON(slug, content, fileMap, slugToId, positions);
    if (node) nodes.push(node);
  }
  if (globalConfig) {
    nodes.push(globalConfig);
  }
  for (const [path, content] of fileMap) {
    if (!path.startsWith("edges/") || !path.endsWith(".md")) continue;
    const edge = parseEdgeToJSON(content, slugToId);
    if (edge) edges.push(edge);
  }
  return { nodes, edges };
}
function parseNodeToJSON(slug, content, fileMap, _slugToId, positions) {
  const { frontmatter, body } = parseFile(content);
  const id = frontmatter.id;
  const type = frontmatter.type;
  const name = frontmatter.name;
  if (!id || !type) return null;
  if (!typeValidator.Check(type)) {
    bclog("exporter:unknown-type", `Node '${name}' (${id}) has unrecognized type '${type}'`);
  }
  const data = {
    name,
    prompt: body || void 0
  };
  if ("isStart" in frontmatter) data.isStart = frontmatter.isStart === true;
  if ("isGlobal" in frontmatter) data.isGlobal = frontmatter.isGlobal === true;
  if ("globalLabel" in frontmatter) data.globalLabel = frontmatter.globalLabel || void 0;
  const tagContent = fileMap.get(`nodes/${slug}/tag.yaml`);
  if (tagContent) {
    const tagData = parseYamlFile(tagContent);
    if (tagData.name) data.tag = tagData;
  } else if (frontmatter.tag) {
    data.tag = frontmatter.tag;
  }
  extractTypeSpecificFields(type, frontmatter, data);
  if (frontmatter.dialogueExamples) {
    data.dialogueExamples = frontmatter.dialogueExamples;
  }
  const conditionContent = fileMap.get(`nodes/${slug}/condition.md`);
  if (conditionContent) {
    const condParsed = parseFile(conditionContent);
    if (condParsed.body) data.condition = condParsed.body;
    if (condParsed.frontmatter.conditionExamples) {
      data.conditionExamples = condParsed.frontmatter.conditionExamples;
    }
  }
  const varsContent = fileMap.get(`nodes/${slug}/variables.yaml`);
  if (varsContent) {
    const varsData = parseYamlFile(varsContent);
    const variables = varsData.variables;
    if (variables && variables.length > 0) {
      data.extractVars = variables.map((v) => {
        const tuple = [
          v.name,
          v.type,
          v.description
        ];
        if (Array.isArray(v._extra)) {
          tuple.push(...v._extra);
        }
        return tuple;
      });
  ...
  const modelContent = fileMap.get(`nodes/${slug}/model.yaml`);
  if (modelContent) {
    const modelData = parseYamlFile(modelContent);
    if (Object.keys(modelData).length > 0) {
      data.modelOptions = modelData;
    }
  }
  const unitTestsContent = fileMap.get(`nodes/${slug}/unit-tests.yaml`);
  if (unitTestsContent) {
    const unitTestsData = parseYamlFile(unitTestsContent);
    if (Object.keys(unitTestsData).length > 0) {
      data.unitTests = unitTestsData;
    }
  }
  const toolsContent = fileMap.get(`nodes/${slug}/tools.yaml`);
  if (toolsContent) {
    const toolsData = parseYamlFile(toolsContent);
    if (toolsData.tools && Array.isArray(toolsData.tools) && toolsData.tools.length > 0) {
      data.tools = toolsData.tools;
    }
  }
...
function extractTypeSpecificFields(type, frontmatter, data) {
  switch (type) {
    case "Webhook": {
      if (frontmatter.url !== void 0) data.url = frontmatter.url;
      if (frontmatter.method !== void 0) data.method = frontmatter.method;
      if (frontmatter.body !== void 0) data.body = frontmatter.body;
      if (frontmatter.headers && Array.isArray(frontmatter.headers)) {
        data.headers = frontmatter.headers.map((h) => [h.key, h.value]);
      }
      if (frontmatter.auth !== void 0) data.auth = frontmatter.auth;
      if (frontmatter.timeoutValue !== void 0) data.timeoutValue = frontmatter.timeoutValue;
      if (frontmatter.max_retries !== void 0) data.max_retries = frontmatter.max_retries;
      if (frontmatter.responseData) data.responseData = frontmatter.responseData;
      if (frontmatter.responsePathways) {
        data.responsePathways = convertResponsePathwaysToTuples(
          frontmatter.responsePathways
        );
      }
      break;
    }
    case "Knowledge Base":
    case "Vector DB Knowledge Base": {
      if (frontmatter.kb !== void 0) data.kb = frontmatter.kb;
      break;
    }
    case "Transfer Call": {
      if (frontmatter.transferNumber !== void 0) data.transferNumber = frontmatter.transferNumber;
      if (frontmatter.warmTransferFields !== void 0) data.warmTransferFields = frontmatter.warmTransferFields;
      if (frontmatter.twilioAppTransferFields !== void 0) data.twilioAppTransferFields = frontmatter.twilioAppTransferFields;
      break;
    }
    case "SMS": {
      if (frontmatter.phoneNumber !== void 0) data.phoneNumber = frontmatter.phoneNumber;
      if (frontmatter.smsBody !== void 0) data.smsBody = frontmatter.smsBody;
      break;
    }
    case "Custom Tool": {
      if (frontmatter.tool !== void 0) data.tool = frontmatter.tool;
      if (frontmatter.speech !== void 0) data.speech = frontmatter.speech;
      if (frontmatter.responsePathways) {
        data.responsePathways = convertResponsePathwaysToTuples(
          frontmatter.responsePathways
        );
    ...
    case "Custom Code": {
      if (frontmatter.code !== void 0) data.code = frontmatter.code;
      if (frontmatter.snippetId !== void 0) data.snippetId = frontmatter.snippetId;
      if (frontmatter.snippetVersion !== void 0) data.snippetVersion = frontmatter.snippetVersion;
      if (frontmatter.snippetVariables !== void 0) data.snippetVariables = frontmatter.snippetVariables;
      break;
    }
    case "Transfer Pathway": {
      if (frontmatter.transferPathway !== void 0) data.transferPathway = frontmatter.transferPathway;
      if (frontmatter.version !== void 0) data.version = frontmatter.version;
      if (frontmatter.transferType !== void 0) data.transferType = frontmatter.transferType;
      if (frontmatter.transferPathwayNode !== void 0) data.transferPathwayNode = frontmatter.transferPathwayNode;
      break;
    }
    case "Press Button": {
      if (frontmatter.mode !== void 0) data.mode = frontmatter.mode;
      if (frontmatter.digit !== void 0) data.digit = frontmatter.digit;
      if (frontmatter.sequence !== void 0) data.sequence = frontmatter.sequence;
      if (frontmatter.digitPrompt !== void 0) data.digitPrompt = frontmatter.digitPrompt;
      break;
    }
    case "Route": {
      if (frontmatter.routes !== void 0) data.routes = frontmatter.routes;
      if (frontmatter.fallbackNodeId !== void 0) data.fallbackNodeId = frontmatter.fallbackNodeId;
      break;
    }
    case "Schedule Meeting":
    case "Scheduling": {
      if (frontmatter.calendarId !== void 0) data.calendarId = frontmatter.calendarId;
      break;
    }
  }
}
function convertResponsePathwaysToTuples(pathways) {
  return pathways.map((rp) => [
    rp.variable,
    rp.condition,
    rp.value,
    { id: rp.targetId, name: rp.targetName }
  ]);
}
function parseEdgeToJSON(content, slugToId) {
  const { frontmatter, body } = parseFile(content);
  const id = frontmatter.id;
  const sourceSlug = frontmatter.source;
  const targetSlug = frontmatter.target;
  if (!id || !sourceSlug || !targetSlug) return null;
  const sourceId = slugToId.get(sourceSlug) ?? sourceSlug;
  const targetId = slugToId.get(targetSlug) ?? targetSlug;
  const edge = {
    id,
    source: sourceId,
    target: targetId,
...
function parsePositions(fileMap) {
  const positions = /* @__PURE__ */ new Map();
  const content = fileMap.get(".pathways/layout.yaml");
  if (!content) return positions;
  const data = parseYamlFile(content);
  const posData = data.positions;
  if (!posData) return positions;
  for (const [slug, pos] of Object.entries(posData)) {
    positions.set(slug, { x: pos.x, y: pos.y });
  }
  return positions;
}
function parseGlobalConfig(fileMap) {
  const mdContent = fileMap.get(".pathways/global_prompt.md");
  if (mdContent?.trim()) {
    return {
      globalConfig: { globalPrompt: mdContent.trim() }
    };
  }
  const yamlContent = fileMap.get(".pathways/config.yaml");
  if (!yamlContent) return null;
  const data = parseYamlFile(yamlContent);
  const globalPrompt = data.globalPrompt;
