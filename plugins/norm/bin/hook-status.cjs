...
async function main() {
	...
	const lines = [
		...
		"Build other parts: /norm:evals (build + run evals), /norm:review (mount + inspect real calls), /norm:tools (custom REST/code tools), /norm:persona (voices + personas + linking), /norm:knowledge (knowledge bases), /norm:triage (issue tracking), /norm:analytics (call analytics + reports), /norm:automations (event-driven triggers + pipelines that place calls).",
