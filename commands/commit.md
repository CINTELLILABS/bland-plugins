...
1. **Validate first.** The workspace should already validate clean. If you are unsure, run `/bland:validate` and resolve all errors before committing. Do not commit a workspace with structural errors. The authoritative validation is `validate_pathway` (the server compiler) — you will also run it directly on the rebuilt graph in step 4a below as the hard pre-commit gate.
...
8. **Refresh the baseline.** On a clean commit, persist the updated version fields (`working_version_number`, bumped `working_revision_number`, and after a publish the advanced `production_version_number`) and re-snapshot the committed graph into `.norm/baseline.json` with native `Write`, so `/bland:status` reflects the newly committed state. Re-read the canonical graph via `POST /v1/convo_pathway/get_one` `{ id: "<pathway_id>", version_number: <working_version_number> }` (full editable graph) for the baseline snapshot; if you also need a refreshed `revision_number`, read `GET /v1/pathway/<pathway_id>/versions` (the only read that returns `revision_number`).
