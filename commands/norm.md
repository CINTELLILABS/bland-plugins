...
Orchestrate the user's Bland agent request through the `norm` agent. You are the entry point; `norm` owns the workflow doctrine (file model, semantics-first, validate-before-ready, auto-commit, drift handling).
...
4. After create/edit/fix work validates with no errors, commit in the same run (`/bland:commit`). Do not stop at a clean local workspace — a clean file tree is not a saved pathway.
