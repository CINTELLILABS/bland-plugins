...
|---|---|---|
| **Claude Code version / namespace** | Plugin MCP servers register as `mcp__bland__*` on some versions and `mcp__plugin_bland_bland__*` on others; hardcoded refs silently pass on one and fail on the other | `/bland:status --check` on a machine with the LATEST Claude Code, installed from the real marketplace |
| **Install path / plugin id** | Zip or local-marketplace installs register under a different qualified id (`bland@bland-local`); anything hardcoding `bland@bland` silently resolves nothing | Install once from a zip mirror; `/bland:status --check` + `the setup skill` must still work |
| **Org / data shape** | A tool can pass on one org's calls and 404 on another's (field-reported: `get_call_log`) | Run `/bland:status --check` under a SECOND org's API key (test org), not just your own |
...
| **Working directory** | Hooks resolve state by walking up from the session's launch dir | Arm `/bland:loop` from a nested subdirectory once |
...
5. Run **`/bland:status --check`** in ONE varied environment from the table (rotate the dimension each release; latest-CC + marketplace install is the highest-yield default).
6. Push, `claude plugin update bland@bland`, restart, spot-check the release's headline feature live.
