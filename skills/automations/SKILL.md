...
- **Event sources (verified live)**: CRM sync events (`salesforce:*`, `hubspot:*` — the `.updated` variants REQUIRE a change-detection condition) and Bland-internal events: `bland:post_call.completed`, `bland:call.started|voicemail_detected|inbound_received|warm_transfer|cold_transfer`, `bland:sms.received`, `bland:batch.completed`, `bland:tool.success|failure`, **`bland:eval.completed`** (scores + per-judge verdicts — automate on failed evals!), **`bland:kb.gap_detected`** (unanswerable question → Slack/webhook), and **`bland:pathway.node_reached`** (fires on EVERY transition — always add a Node Name condition). These internal events are how the quality flywheel automates itself.
- **`_changes` is the change-detection ground truth**: `.updated` event payloads carry `_changes: { field: { previous, current } }` — that object is what `changed`/`changed_to`/`changed_from` operators evaluate; when a filter misbehaves, compare it against a real execution's `input._changes`.
...
- **KNOWN GAP (verified live, 3/3 deterministic)**: POST writes to `/v1/automation/*` currently fail with `INVALID_JSON` through the `call_bland_api` passthrough — even with correct native-object bodies — while every GET works and the same tool POSTs fine to all other routers. Until the platform fixes the router↔loopback interplay: do ALL reads/monitoring/diagnosis here fully, but for creates, updates, toggles, and trigger tests, report the gap plainly and direct the user to the Automations UI in the dashboard. Do NOT retry variations hoping to sneak past it, and never claim a write worked without a read-back.
...
**4. Monitor: run history, results, health.** The execution log is the automation's flight recorder — read it proactively, not just on failure:

- **Trigger-level history at a glance (verified live)**: the triggers LIST already carries `trigger_count`, `last_triggered_at`, `action_type`, and `timing_mode` per trigger — answer "has this ever fired / when last" from the list before touching executions.
- **Run history views** (all free reads, paginated `limit`/`offset`): per-trigger — `GET /executions?trigger_id=<id>` ("what has this trigger done lately"); per-pipeline — `?event_pipeline_id=<id>`; failures-first — `?status=failed`; in-flight — `?status=running` (stuck executions show here; `pending` that never turns `running` means the workflow never started). Page back far enough to cover the window the user asked about — the default page is small; state how far the history you read actually reaches.
- **Reading a run's RESULTS**: `GET /executions/{id}` → `input` is the exact normalized event the trigger received (the ground truth for "what data did we get"); `result` is the accumulated per-node output map — this is where a `send_call` node's placed call id, an SMS node's message sid, and every fetch node's data live; `nodes_executed` shows the path taken (a conditions node that stopped the pipeline shows as the last entry); `duration_ms` + `started_at`/`completed_at` give timing. Quote from these fields — never summarize a run from its status alone.
- **Health report pattern** (when the user asks "how are my automations doing"): pipelines via `GET /pipelines?include_stats=true` for the per-pipeline overview, then per pipeline a recent-executions sample → report fired count, success/failure split, last failure's `error` verbatim, and median duration. A pipeline with zero recent executions is a finding too (trigger inactive? event never firing? filter too strict — dry-run it).
- **From runs to calls**: harvest placed-call ids from each execution's `result` and hand cohorts to the call-review skill (call quality) or the analytics skill (trends) — automation health and call health are different questions; answer both when the user asks "is it working".

**5. Diagnose an execution.** `GET /executions?status=failed` → detail: `error` names the failing step, `nodes_executed` shows how far it got, `input` shows what the trigger actually received (the #1 root cause: the payload didn't contain what the filter or template expected), `result` carries per-node outputs including placed call ids. Trace the call side via the id in `result` (or the call's `metadata.automation_execution_id`) and hand call-level forensics to the call-review skill. A trigger that never fired at all → re-check the filter against a real payload with `dryRun: true`; a trigger that fired too much → the filter is missing a change-detection operator.

**6. Trace an automated call back.** Given a call with `metadata.automation_execution_id`: fetch the execution, and you have the whole story — which event, which conditions passed, which node placed it, with what request_data.
...
Report: what exists (pipelines + triggers with active states), what changed (ids + read-back), test results (per-condition outcomes; execution id + status for live runs), and for diagnoses the full chain — event input → conditions → nodes executed → error or placed-call ids — with the call-review skill handoff when call-level analysis is next.
