...
- **Recommended voices** — call `list_voices` when choosing a voice for a persona, inbound number, call, or pathway. It returns Bland's curated voices on the server's recommended generation, with ids, descriptions, tags, and public `service` labels. Match the character brief within that set; curation alone does not identify the recommended generation.
- **Browse public alternatives** — `GET /v1/voices/shared` supports `service`, `search`, `tag`, `page`, and `page_size`. Read `GET /v1/models` for `is_recommended` and supported languages/features; use its `model_id` as the service filter. Choose a compatible model when the recommended one does not meet the user's requirements, and explain the tradeoff. Tags match any supplied tag, so verify curation separately when combining tag filters. Do not fetch the unbounded `/v1/voices` catalog for recommendations.
