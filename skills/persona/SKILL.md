...
- **Browse public alternatives** — `GET /v1/voices/shared` supports `service`, `search`, `tag`, `page`, and `page_size`. Use `list_voices` for default recommendations; consult `GET /v1/models` for supported languages/features and use its `model_id` as the service filter. Choose a compatible model when the recommended set does not meet the user's requirements, and explain the tradeoff. Tags match any supplied tag, so verify curation separately when combining tag filters. Do not fetch the unbounded `/v1/voices` catalog for recommendations.
