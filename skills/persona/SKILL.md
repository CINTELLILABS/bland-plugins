...
- **Recommended voices** — call `list_voices` when choosing a voice for a persona, inbound number, call, or pathway. It returns Bland's curated voices on the current, recommended generation, with ids, descriptions, tags, and `service` (the generation; same value as `model_id` on `GET /v1/models`). Match the character brief within that set.
- **Browse public alternatives** — `GET /v1/voices/shared` supports `service`, `search`, `tag`, `page`, and `page_size`. Use `list_voices` for default recommendations; consult `GET /v1/models` for supported languages/features (`is_recommended` marks the current generation) and pass its `model_id` as the `service` filter. When compatibility points at an older generation, explain the tradeoff and let the user choose. Tags match any supplied tag, so verify curation separately when combining tag filters. Do not fetch the unbounded `/v1/voices` catalog for recommendations.
