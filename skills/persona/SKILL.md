...
- **Read and write persona resources through the generic passthrough** (voice recommendations use `list_voices`):
  - `bland_api_get` for REST reads (`GET`) — browsing voice catalogs, listing/getting personas, listing/getting persona versions.
...
- **Recommended voices** — call `list_voices` when choosing a voice for a persona, inbound number, call, or pathway. It returns Bland's curated voices on its current generation, with ids, descriptions, and tags. Match the character brief within that set; curation alone does not identify the current generation.
- **Browse public alternatives** — `GET /v1/voices/shared` supports `service`, `search`, `tag`, `page`, and `page_size`. Use `list_voices` for default recommendations; consult `GET /v1/models` for supported languages/features and use its `model_id` as the service filter. When compatibility requires an older generation, explain the tradeoff and get the user's approval before selecting it. Tags match any supplied tag, so verify curation separately when combining tag filters. Do not fetch the unbounded `/v1/voices` catalog for recommendations.
- **Private or saved voices** — `GET /v1/voices/library` lists the caller's own voices. The shared catalog excludes private clones. Honor an explicitly requested voice even when absent from recommendations; resolve an ambiguous name before changing it.
...
For a new voice choice, call `list_voices`, match the intended character by description/tags, and use the selected `id` as `call_config.voice`. Honor an explicitly requested voice, including a private clone or older voice. During unrelated edits, keep the existing voice. Use the public or private discovery paths above only when the recommended list cannot meet the request. Never invent a voice id.

New MCP persona configurations default to a live curated V3 voice. Assigning a non-V3 voice requires `allow_non_v3_voice: true` at the top level of `call_bland_api`, outside `body`, only after the user explicitly selects that voice. An unrelated edit can preserve an existing older voice, but placing a call with it requires the user's approval and the same flag. Never set the flag merely to bypass a validation error.

### Set the call config and the prompt so the persona sounds human
`call_config` decides how the persona sounds before it says a word: set `background_track` (`office` unless the brief wants a silent line), and send `noise_cancellation: true` explicitly on the calls that use the persona. Write the `personality_prompt` the way people actually talk, not like written copy. The dispatch settings, Bland's tone rubric, and the performance-tag grammar (check support for the selected voice; recommendation does not guarantee feature support) are in the calls skill's `references/sounding-human.md` — read it before you write or edit a `personality_prompt`.
...
`POST /v1/personas/{id}/versions/promote` (it archives the previous production version) and `DELETE /v1/personas/{id}` are high-impact — ask for explicit confirmation, naming the persona by id and name and stating exactly what will change, before running them. Removing a `pathway_conditions` entry changes live routing on the draft — call it out and confirm. Any write that mutates production, makes real outbound calls, sends messages, or costs money likewise needs explicit confirmation first. Read-only inspection (`list_voices`, `GET /v1/voices/shared`, `GET /v1/voices/library`, `GET /v1/personas*`, the version reads, the docs tools) is safe and never needs confirmation.
...
4. Keep the existing voice unless the user wants a change. For a new choice, use `list_voices` and the voice-selection guidance above; honor a specific voice or private clone. Set the chosen id as `call_config.voice`.
