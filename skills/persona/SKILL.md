...
- **List voices** — `GET /v1/voices`. Returns `{ voices: [...] }`; each voice has `id`, `name`, `description`, `public`, `tags` (the `Bland Curated` tag marks the voices recommended for phone quality), and rating fields. **The list is large (about 1,000 voices, ~400 KB) and the endpoint has no `limit` or filter, so the raw result overflows a tool response.** When the host saves an oversized result to a file, filter that file instead of re-requesting: `jq -c '.data.data.voices[] | select(.service == "BTTS_V3" and .tags and (.tags|index("Bland Curated"))) | {id,name,description}'` gives the same curated set as `list_voices` in a few KB. Better still, ask the user for a voice name or character first and read only that voice with `GET /v1/voices/{voice_id}`, or keep the current `call_config.voice` when editing. Pick by description/tags and use the voice `id` as `call_config.voice`.
