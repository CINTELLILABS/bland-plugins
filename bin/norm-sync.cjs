 ...
 * /bland:* command + agent bodies. The local files under pathway/ are the REAL
...
function generateNodeMd(node) {
	...
	// (These are typed config edited via the /bland:* commit flow, not hand-edited prose.)
...
//
// Kept as a pure offline helper: the the call-review skill flow fetches a call via the MCP
		...
		switch (sub) {
			...
			case "-h":
				ok({
					...
					note: "No network, no credentials. ALL server I/O goes through the MCP passthrough (mcp__bland__bland_api_get / call_bland_api) in the /bland:* command + agent bodies. See bin/SYNC.md.",
				});
				break;
			default:
