...
/**
 * Bland — credential resolver for the DESKTOP STDIO BRIDGE ONLY.
 *
 ...
 *      pluginConfigs["bland@bland"].options.bland_api_key / .bland_api_url
 ...
 *      stray top-level key or a sibling plugin like "bland@bland-local" can't win)
...
// "bland@bland-local"), so nothing below may assume this exact id — resolution
// prefers it but falls back to any pluginConfigs entry whose id starts with
// "bland@", then the pre-2.0 "norm@" ids (so an upgrade keeps its key), and
// finally any entry that actually carries our option keys.
const PLUGIN_ID = "bland@bland";
const LEGACY_PLUGIN_ID = "norm@bland";
const isOurId = (id) => id.startsWith("bland@") || id.startsWith("norm@");
...
function exactPluginOption(root, option) {
	...
	const ids = [
		...
		...Object.keys(configs).filter(
			(id) => id !== PLUGIN_ID && id.startsWith("bland@"),
		),
		...Object.keys(configs).filter((id) => id.startsWith("norm@")),
		...Object.keys(configs).filter((id) => !isOurId(id)),
	];
	for (const id of ids) {
		const opts = configs[id] && configs[id].options;
		if (!opts) continue;
		// Foreign ids only count when they demonstrably hold OUR config shape.
		if (!isOurId(id) && !(KEY_OPTION in opts || URL_OPTION in opts))
			continue;
...
function fromKeychain(option) {
	...
	const services = ["Claude Code-credentials", "Claude Code", PLUGIN_ID, LEGACY_PLUGIN_ID];
	const accounts = [
		option, // bland_api_key
		`${PLUGIN_ID}:${option}`, // bland@bland:bland_api_key
		`${PLUGIN_ID}.${option}`, // bland@bland.bland_api_key
		PLUGIN_ID, // bland@bland
		`${LEGACY_PLUGIN_ID}:${option}`,
		`${LEGACY_PLUGIN_ID}.${option}`,
		LEGACY_PLUGIN_ID,
	];
...
/**
 * The Bland CLI's saved profile (`bland auth login`, including the device-code
 * flow). The CLI stores it with the `conf` package: project "bland-cli", suffix
 * "nodejs", plain JSON. Sharing this file means one login serves the CLI, the
 * plugin's bin tools, and (once it forwards to the hosted server) `bland mcp`.
 * Returns { api_key, base_url } of the current profile, or null.
 */
function fromCliProfile() {
	const os = require("node:os");
	const home = os.homedir();
	const candidates = [];
	if (process.platform === "darwin") {
		candidates.push(path.join(home, "Library", "Preferences", "bland-cli-nodejs", "config.json"));
	} else if (process.platform === "win32") {
		const appData = process.env.APPDATA || path.join(home, "AppData", "Roaming");
		candidates.push(path.join(appData, "bland-cli-nodejs", "Config", "config.json"));
	} else {
		const xdg = process.env.XDG_CONFIG_HOME || path.join(home, ".config");
		candidates.push(path.join(xdg, "bland-cli-nodejs", "config.json"));
	}
	for (const f of candidates) {
		const root = readJson(f);
		if (!root || typeof root !== "object") continue;
		const name = nonEmptyString(root.current_profile) || "default";
		const profile = root.profiles && root.profiles[name];
		if (!profile || typeof profile !== "object") continue;
		const apiKey = nonEmptyString(profile.api_key);
		if (!apiKey) continue;
		return { api_key: apiKey, base_url: nonEmptyString(profile.base_url) };
	}
	return null;
}
