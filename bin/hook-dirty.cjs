...
/**
 * Bland — Stop hook: NUDGE when the pathway workspace has uncommitted edits.
 *
 ...
 * /bland:validate then /bland:commit.
 *
...
function normSyncBin() {
	...
	if (!root) return null;
	// ${CLAUDE_PLUGIN_ROOT} resolves to the plugin dir (the plugin root); the
...
async function main() {
	...
	try {
		...
		emit(
			`Bland: your pathway workspace has ${changed.length} uncommitted ` +
				`edit${changed.length === 1 ? "" : "s"} (${shown}${more}). ` +
				`Run /bland:validate then /bland:commit to save — an uncommitted ` +
