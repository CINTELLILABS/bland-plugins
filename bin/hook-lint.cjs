...
/**
 * Bland — PostToolUse hook (offline frontmatter/edge lint).
 *
...
function lintPathwayFile(workspaceRoot, filePath) {
	...
	if (inNodes && base === "node.md") {
		...
		notes.push(
			`${rel}: structured YAML — the file IS the edit surface. Keep the JSON-inlined frontmatter parseable, quote <digits>e<digits> slugs, check the shape with get_pathway_schema when unsure, and run /bland:validate before commit.`,
		);
...
async function main() {
	...
	const context = ["Bland lint (advisory, non-blocking):", ...notes.map((n) => `- ${n}`)].join("\n");
	try {
