...
/**
 * Bland — Stop hook: convergence-loop gate (evaluator-optimizer).
 *
 * When a /bland:loop is active for THIS session, refuse to let the turn end while
...
async function main() {
	...
	const reason = [
		...
		"3. Commit the clean graph: /bland:commit (the only confirm-gated write).",
