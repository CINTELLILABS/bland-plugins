...
server goes through the MCP tools driven from the `/bland:*` command + agent bodies:
...
so `/bland:status` and `/bland:commit` can diff local vs. baseline without a
...
| read call | `GET /v1/calls[/:id]` | `bland_api_get` / `get_call_log` | for the call-review skill; feed payload to `generateCallFiles` offline |
